...
async function debugContanti() {
  const supabase = createClient(supabaseUrl, supabaseServiceKey)
  const centroId = '1a72344b-aac1-465b-92d4-7e670f430340'

  try {
    console.log('\n🔍 DEBUG: Movimenti Contanti\n')
    console.log('='.repeat(80))

    // 1. Trova movimenti con "versamento" e "contanti"
    console.log('\n1️⃣  Movimenti con "VERSAMENTO CONTANTI":')
    const { data: movements, error: movError } = await supabase
      .from('bank_movements')
      .select('id, data, descrizione, categoria')
      .eq('centro_id', centroId)
      .ilike('descrizione', '%versamento%contanti%')
    ...
    // 2. Controlla regole vendor
    console.log('\n\n2️⃣  Regole Vendor (in ordine):')
    ...
    // 3. Test pattern matching su un movimento esempio
    if (movements && movements.length > 0) {
      const testMov = movements[0]
      console.log('\n\n3️⃣  Test Pattern Matching:')
      console.log(`   Movimento test: "${testMov.descrizione.substring(0, 60)}..."`)
      ...
      for (const vendor of vendors) {
        ...
        if (matched) {
          ...
          console.log(`      🔴 QUESTO SAREBBE APPLICATO (loop si ferma qui)`)
          break
        } else {
