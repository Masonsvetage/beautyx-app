...
const ALLOWED_ROLES = ['titolare', 'direttore', 'amministrativo', 'admin']
...
export async function POST(request) {
  try {
    ...
    if (!profile?.centro_id) {
      return Response.json({ error: 'Nessun centro associato al tuo profilo' }, { status: 403 })
    }
    if (!ALLOWED_ROLES.includes(profile.ruolo_livello)) {
    ...
    if (!tipo) {
      return Response.json({
        error: 'Tipo file non riconosciuto. Carica un export Koibox valido (Clienti, Casse, Servizi o Appuntamenti).',
      }, { status: 400 })
    }
    ...
    const result = await importTable(adminClient, TABLE_MAP[tipo], records, profile.centro_id)
    ...
    console.error('[KOIBOX USER IMPORT]', error)
