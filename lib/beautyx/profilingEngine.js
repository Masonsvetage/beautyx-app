...
import { callClaudeWithFallback, PRIMARY_MODEL } from './callClaudeWithFallback'
...
// La chiamata Anthropic per l'analisi del testo libero del profiling (task
// #152) resta dedicata e separata dal loop tool della chat gestionale (vedi
// piano-sviluppo-report-care.md punto 6) — ma ora passa dall'helper
// centralizzato lib/beautyx/callClaudeWithFallback.js (client Anthropic
// istanziato lì), che aggiunge il fallback automatico su un secondo modello
// se il primario non è disponibile (vedi memory/davide.md 2026-09-19).
 ...
 * Chiamata Anthropic dedicata (passa da callClaudeWithFallback.js, separato dal
 * loop tool della chat gestionale), con prompt strutturato che restituisce SOLO
 * JSON. Coerente col
...
export async function analizzaNarrazioneLibera(input) {
  ...
  return {
    ...
    // Riflette il modello REALMENTE usato (primario o fallback, vedi
    // callClaudeWithFallback.js) invece di un valore hardcoded — prima di
    // questo fix restava sempre 'claude-sonnet-5' anche quando la chiamata
    // era stata servita dal fallback.
    _model: resp?._modelUsed || PRIMARY_MODEL,
  }
}
